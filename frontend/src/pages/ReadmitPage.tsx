...
export default function ReadmitPage({ onSelect, setNotice, onReadmitComplete, ocrLanguage }: Props) {
  ...
  const handleReadmit = async () => {
    ...
    try {
      ...
      setNotice({
        type: "success",
        message: `Re-admitted. Admission ${data.admission_id}. Patient added to OP Queue Management.`,
      });
      setNotes("");
      setDocFiles({});
      setOcrResults({});
                ...
                {expanded && activePatient && (
                  <div className="table-row-expand">
                    <div className="readmit-form">
                      <h4>
                        Re-admitting: {activePatient.name} {activePatient.last_name} ({activePatient.patient_id})
                      </h4>
                      <Label>
                        Admission Notes
