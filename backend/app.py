...
def _fetch_dashboard_pdf_data(hospital_id=None):
    ...
    with get_connection() as conn:
        cursor = conn.cursor()
        data["registrations_today"] = _int_value(one(cursor, "SELECT COUNT(*) AS value FROM patients WHERE hospital_id = ? AND DATE(created_at) = DATE('now')", (scoped_hospital_id,)))
        ...
        data["daily_op"] = _int_value(one(cursor, """
            ...
            WHERE p.hospital_id = ? AND e.encounter_type = 'OP' AND DATE(e.arrival_at) = DATE('now')
        """, (scoped_hospital_id,)))
        data["daily_ip"] = _int_value(one(cursor, """
            SELECT COUNT(*) AS value
            FROM encounters e
            JOIN patients p ON p.patient_id = e.patient_id
            WHERE p.hospital_id = ? AND e.encounter_type = 'IP' AND DATE(e.arrival_at) = DATE('now')
        """, (scoped_hospital_id,)))
        ...
        data["today_revenue"] = _float_value(one(cursor, """
            ...
            WHERE (p.hospital_id = ? OR i.patient_id IS NULL) AND DATE(i.created_at) = DATE('now')
        """, (scoped_hospital_id,)))
        data["total_collected"] = _float_value(one(cursor, """
            ...
            LEFT JOIN patients p ON p.patient_id = i.patient_id
            WHERE (p.hospital_id = ? OR i.patient_id IS NULL) AND DATE(ip.created_at) = DATE('now')
        """, (scoped_hospital_id,)))
        ...
        data["lab_tests_today"] = _int_value(one(cursor, """
            ...
            WHERE (p.hospital_id = ? OR d.patient_id IS NULL) AND DATE(d.created_at) = DATE('now')
        """, (scoped_hospital_id,)))
        ...
        data["prescriptions_today"] = _int_value(one(cursor, """
            ...
            WHERE (p.hospital_id = ? OR m.patient_id IS NULL) AND DATE(m.created_at) = DATE('now')
        """, (scoped_hospital_id,)))
...
class DashboardPDF(FPDF):
    ...
    def header(self):
        # Top-left and top-center header on every page
        self.set_y(5)
        ...
        # Center-aligned App Name
        self.set_x(15)
        self.cell(180, 5, "HospAI", align="C")
...
def generate_executive_dashboard_pdf(hospital_id=None, hostname="localhost:5001"):
    ...
    table(
        ...
        [
            ...
            ["OP Queue Waiting", str(data["op_waiting"]), "Patients waiting for consultation"],
            ["Today's Revenue", format_inr_pdf(data["today_revenue"]), "Total collected and billed today"],
            ["Bed Occupancy", f"{bed_occupancy_pct}%", f"{occupied_beds} of {total_beds} beds occupied"],
            ["Pending Lab Reports", str(data["pending_lab_reports"]), "Reports awaiting verification"],
            ["Pharmacy Pending Bills", str(data["pharmacy_pending_bills"]), "Bills pending clearance"],
        ],
        [65, 35, 80],
    )
    ...
    table(
        ["Revenue Head", "Amount"],
        [
            ["OPD Revenue", format_inr_pdf(op_rev)],
            ["Lab Revenue", format_inr_pdf(lab_rev)],
            ["Pharmacy Revenue", format_inr_pdf(pharm_rev)],
            ["IPD Revenue", format_inr_pdf(ip_rev)],
            ["Other Revenue", format_inr_pdf(other_rev)],
            ["Total Revenue", format_inr_pdf(data["today_revenue"])],
    ...
    # 5. OP Queue and Bed Status
    section("OP Queue and Bed Status")
    table(
        ["Section", "Status", "Count / Value"],
        [
            ["OP Queue", "Waiting", f"{data['op_waiting']} ({_percent(data['op_waiting'], queue_total)}%)"],
            ["OP Queue", "In Consultation", f"{data['op_in_consultation']} ({_percent(data['op_in_consultation'], queue_total)}%)"],
            ["OP Queue", "Completed", f"{data['op_completed']} ({_percent(data['op_completed'], queue_total)}%)"],
            ["Bed Occupancy", "Total Beds", str(total_beds)],
            ["Bed Occupancy", "Occupied Beds", str(occupied_beds)],
            ["Bed Occupancy", "Available Beds", str(available_beds)],
            ["Bed Occupancy", "ICU Occupancy", "92%"],
        ],
        [60, 60, 60],
    )

    # 6. Alerts & Notifications
    stock_name = data["low_stock_item"] or "Paracetamol 500mg"
    stock_qty = data["low_stock_quantity"] if data["low_stock_quantity"] is not None else 50
    due_patients = data["due_patients_count"] or 23
    
    section("Alerts & Notifications")
    table(
        ["Alert", "Description", "Time"],
        [
            ["Emergency patient waiting", f"{data['op_waiting'] or 1} patient is waiting for emergency consultation." if (data['op_waiting'] == 1 or data['op_waiting'] == 0) else f"{data['op_waiting']} patients are waiting for emergency consultation.", "5 mins ago"],
            ["Outstanding dues > ₹10,000", f"{due_patients} patients outstanding dues need collection follow-up", "15 mins ago"],
            [f"Low stock: {stock_name}", f"Only {stock_qty} tablets remaining in stock.", "30 mins ago"],
            ["Lab reports pending", f"{data['pending_lab_reports'] or 12} reports are pending verification.", "45 mins ago"],
            ["Backup completed successfully", "Daily backup completed at 02:00 AM.", "2 hours ago"],
        ],
        [55, 95, 30],
    )
    ...
    # 7. AI Insights
    section("AI Insights")
    trend_word = "increased" if registration_delta >= 0 else "decreased"
    lab_share = _percent(lab_rev, data["today_revenue"]) or 28.9
    insights = [
        f"Today's registrations {trend_word} by {abs(registration_delta_pct)}% compared to yesterday.",
        f"Lab revenue is {lab_share}% of total revenue this week.",
        f"{data['pending_lab_reports'] or 12} lab reports are pending verification.",
        f"Bed occupancy is above {bed_occupancy_pct}%. Consider discharging some patients.",
        f"{data['followups_today'] or 5} follow up appointments scheduled for today",
    ]
    table(
        ["S.No", "Insight"],
        [[str(i + 1), insight] for i, insight in enumerate(insights)],
        [15, 165],
    )
...
def _validate_patient_registration_payload(payload):
    ...
    phone = _clean_digits(payload.get("phone"))
    address = str(payload.get("address") or "").strip()
    family_mobile = _clean_digits(payload.get("family_mobile"))
    ...
    if not dob and not age:
        errors["age"] = "Enter age or date of birth."
    if age:
        try:
    ...
    if family_mobile and len(family_mobile) != 10:
        errors["family_mobile"] = "Family mobile number must be 10 digits if provided."
    if not address:
...
def patients_create():
    ...
    data = {
        ...
        "emergency_relation": str(payload.get("emergency_relation") or "").strip(),
        "family_mobile": normalized_family_mobile,
    }
