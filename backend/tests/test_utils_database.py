...
def test_generate_patient_id_sequence():
    ...
    seed = ((now.year % 100) * 372) + (now.month * 31) + iso_week + int(now.strftime("%d"))
    period_block_start = ((seed % 89) + 10) * 100 + 1
    assert first_id.startswith(f"PAT-{date_str}-")
    assert first_id.endswith(str(period_block_start))
    ...
    assert second_id.endswith(str(period_block_start + 1))
